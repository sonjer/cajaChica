...
CancelarReq = function(data_str) {
	...
	angular.element($('#requisicionID')).scope().delete();
	$("#contenido tr").remove();
	$("#obra").focus();
}
...
GuardarReq = function(data_str) {
	$.ajax({
		type : 'post',
		url : urlRequisiciones + 'updateReq',
		data : {
...
function buscarCliente(id) {
	...
	$.getJSON(url, function(data) {
		$.each(data, function(i) {
			...
			$("#cliente_id").val(ccostos[0][0]['idCliente']);
			$("#cliente").val(ccostos[0][0]['idCliente'] + ' | ' + ccostos[0][0]['nombre']);
		});
	}).fail(function() {
		//   alert("FAIL "+ url);
		$("#cliente").val('');
